...
public class ControlElement {
    ...
    private boolean lock8Way = false;
    ...
    public boolean isLock8Way() {
        return lock8Way;
    }

    public void setLock8Way(boolean lock8Way) {
        this.lock8Way = lock8Way;
    }
        ...
        switch (type) {
            case BUTTON: {
                ...
                if (pressed) {
                    ...
                    paint.setColor(Color.BLACK); // Use opaque color for DST_OUT to ensure full transparency in the hole
                }

                if (iconId > 0) {
                    drawIcon(canvas, cx, cy, boundingBox.width(), boundingBox.height(), iconId);
                }
                else if (getBindingAt(0) == Binding.ANDROID_KEYBOARD) {
                    drawIcon(canvas, cx, cy, boundingBox.width(), boundingBox.height(), 15);
                }
                else {
            ...
            case D_PAD: {
                ...
                for (int i = 0; i < 4; i++) dpadPaths[i] = new Path();
    ...
    private void drawIcon(Canvas canvas, float cx, float cy, float width, float height, int iconId) {
        ...
        if (!pressed) paint.setColorFilter(inputControlsView.getColorFilter());
    ...
    public JSONObject toJSONObject() {
        try {
            ...
            elementJSONObject.put("lock8Way", lock8Way);
            ...
            if (type == Type.STICK) {
                if (lock8Way) {
                    ...
                    if (magnitude > STICK_DEAD_ZONE) {
                        ...
                        angle = (float)(Math.round(angle / (Math.PI / 4.0)) * (Math.PI / 4.0));
                        deltaX = (float)Math.cos(angle) * magnitude;
                        deltaY = (float)Math.sin(angle) * magnitude;
                    }
                    else {
                        deltaX = 0;
                        deltaY = 0;
