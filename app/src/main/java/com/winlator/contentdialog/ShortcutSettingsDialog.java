...
public class ShortcutSettingsDialog extends ContentDialog {
    ...
    private void createContentView() {
        ...
        cbEnableXInput.setChecked(shortcut.getExtra("enableXInput", shortcut.container.isEnableXInput() ? "1" : "0").equals("1"));

        final CheckBox cbEnableDInput = findViewById(R.id.CBEnableDInput);
        cbEnableDInput.setChecked(shortcut.getExtra("enableDInput", shortcut.container.isEnableDInput() ? "1" : "0").equals("1"));

        final LinearLayout llDInputMapperType = findViewById(R.id.LLDInputMapperType);
        final Spinner sDInputMapperType = findViewById(R.id.SDInputMapperType);
        sDInputMapperType.setSelection(Byte.parseByte(shortcut.getExtra("dinputMapperType", String.valueOf(shortcut.container.getDInputMapperType()))));

        cbEnableXInput.setOnCheckedChangeListener((buttonView, isChecked) -> {
            if (isChecked) cbEnableDInput.setChecked(false);
        });

        cbEnableDInput.setOnCheckedChangeListener((buttonView, isChecked) -> {
            if (isChecked) cbEnableXInput.setChecked(false);
            llDInputMapperType.setVisibility(isChecked ? View.VISIBLE : View.GONE);
        });
        llDInputMapperType.setVisibility(cbEnableDInput.isChecked() ? View.VISIBLE : View.GONE);
        ...
        setOnConfirmCallback(() -> {
            ...
            if (!shortcut.name.equals(name) && !name.isEmpty()) {
                ...
                boolean enableXInput = cbEnableXInput.isChecked();
                boolean enableDInput = cbEnableDInput.isChecked();
                int dinputMapperType = sDInputMapperType.getSelectedItemPosition();

                shortcut.putExtra("enableXInput", enableXInput != shortcut.container.isEnableXInput() ? (enableXInput ? "1" : "0") : null);
                shortcut.putExtra("enableDInput", enableDInput != shortcut.container.isEnableDInput() ? (enableDInput ? "1" : "0") : null);
                shortcut.putExtra("dinputMapperType", dinputMapperType != shortcut.container.getDInputMapperType() ? String.valueOf(dinputMapperType) : null);
